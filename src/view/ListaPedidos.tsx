...
import { LocalStorageKeys } from "../Constants/index.ts";
...
export function ListaPedidos() {
  ...
  const eliminarProducto = (producto: Order) => {
    ...
    localStorage.setItem(
      ...
      JSON.stringify(listaProductosFiltrada)
    );
  };
  ...
  useEffect(() => {
    ...
    const listaDeOrders: Order[] = listaDeProductos.map((producto: Product) => {
      const pedidoExistente = listaDePedidos.find(
        (order: Order) => order.name === producto.name
      );
  ...
  return (
    <div className="container">
      ...
      <button className="export-pdf-button" onClick={generarPDF}>
        Exportar a PDF
      </button>
    </div>
  );
